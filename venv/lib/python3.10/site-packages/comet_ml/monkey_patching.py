...
def _reset_already_imported_modules():
    # Modify the set in place to be sure to keep the same reference everywhere
...
class CustomFileLoader(object):
    ...
    def exec_module(self, module):
        ...
        try:
            ...
            LOGGER.debug(
                "Failed to patch {}".format(self.fullname),
                exc_info=True,
            )
            return module
...
class ExceptionCaughtSentinel:
    def __init__(self, exception):
        self.exception = exception

    def __eq__(self, other):
        if not isinstance(other, ExceptionCaughtSentinel):
            return False
        return self.exception is other.exception
...
def Entrypoint(original, before=None, after=None, allow_empty_experiment=False):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            return_value = ExceptionCaughtSentinel(exception)

        # Call after callbacks once we have the return value
        if should_run:
            for callback in after_callbacks:
                callback_allows_exception = getattr(
                    callback, "allow_after_exception", False
                )
                skip_callback = (
                    exception_raised is not None and not callback_allows_exception
                )
                if skip_callback:
                    continue
...
def is_error_sentinel(value):
    return isinstance(value, ExceptionCaughtSentinel)
