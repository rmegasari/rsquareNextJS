...
class FakeStd(object):
    ...
    def __init__(self, handler, original, isatty):
...
class BaseStdWrapper(object):
    ...
    def __enter__(self):
        if self.stdout and self.stdout_handler:
            self._stdout = FakeStd(self.stdout_handler, sys.stdout, _isatty(sys.stdout))
            self._old_stdout = sys.stdout
            sys.stdout = self._stdout

        if self.stderr and self.stderr_handler:
            self._stderr = FakeStd(self.stderr_handler, sys.stderr, _isatty(sys.stderr))
...
class Unbuffered(object):
    ...
    def __init__(self, stream, isatty):
    ...
    class PipeMultiprocessingStdWrapper(object):
        ...
        def __enter__(self):
            ...
            if self.stdout:
                ...
                sys.stdout = Unbuffered(sys.stdout, isatty)
        ...
        def __exit__(self, a=None, b=None, c=None):
            ...
            if self.consumer_thread:
                ...
                if self.consumer_thread.is_alive():
                    LOGGER.warning(CONSOLE_CONSUMER_THREAD_DID_NOT_FINISH_WARNING)
