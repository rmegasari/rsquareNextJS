...
from .assets.assets_helper import asset_download_rest_path
...
class Embedding(object):
    ...
    def __init__(
        ...
        vector_asset_id: Optional[str] = None,
        metadata_asset_id: Optional[str] = None,
        sprite_asset_id: Optional[str] = None,
        experiment_key: Optional[str] = None,
    ):
        """
        The URL's and sizes for all Embedding's resources.
        """
        if vector_url is None and vector_asset_id is None:
            raise ValueError("Either vector_url or vector_asset_id must be provided")

        if metadata_url is None and metadata_asset_id is None:
            raise ValueError(
                "Either metadata_url or metadata_asset_id must be provided"
            )
        ...
        self.title = title
        self.vector_asset_id = vector_asset_id
        self.metadata_asset_id = metadata_asset_id
        self.sprite_asset_id = sprite_asset_id
    ...
    def to_json(self):
        ...
        """
        if self.vector_url is not None:
            tensor_path = self.vector_url
        else:
            tensor_path = asset_download_rest_path(
                asset_id=self.vector_asset_id, experiment_key=self.experiment_key
            )

        if self.metadata_url is not None:
            metadata_path = self.metadata_url
        else:
            metadata_path = asset_download_rest_path(
                asset_id=self.metadata_asset_id, experiment_key=self.experiment_key
            )

        image_path = None
        if self.sprite_url is not None:
            image_path = self.sprite_url
        elif self.sprite_asset_id is not None:
            image_path = asset_download_rest_path(
                asset_id=self.sprite_asset_id, experiment_key=self.experiment_key
            )
...
class Histogram(object):
    ...
    See also [comet_ml.CometExperiment.log_histogram][]
    """
    ...
    def from_json(cls, data, verbose=True):
        ...
        if isinstance(data, str):
            filename = expand_user_home_path(data)
