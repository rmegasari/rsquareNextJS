...
def multi_gpu_model_wrapper(experiment, original, model, result, *args, **kwargs):
    try:
        experiment._storage["keras"]["json_model"]["gpu_model_%s" % id(model)] = (
...
def patch(module_finder):
    ...
    # TF >= 2.16
    module_finder.register_before("keras.api.models", "Model.fit", fit_keras_logger)
    module_finder.register_before(
        "keras.api.models", "Model.fit_generator", fit_keras_logger
    )
    ...
    # Probably both TF < 2.16 and >= 2.16
    module_finder.register_before(
