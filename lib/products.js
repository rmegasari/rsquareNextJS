...
import { getAllProducts as getAllProductsFromDB, getProductById as getProductByIdFromDB } from "./db-wrapper";
...
export async function getProductById(id, options = { includeInactive: false }) {
  // Try database first (auto-switches: Supabase > Postgres > SQLite)
  if (USE_DATABASE) {
    try {
      ...
      if (product && !options.includeInactive) {
        // If product is explicitly inactive (active = false), return null
  ...
  if (USE_DATABASE) {
    try {
      ...
      if (products && products.length > 0) {
        // Products are already sorted by order_number in the database query
        return products;
      }
    } catch (error) {
      console.warn("Database error, falling back to JSON:", error.message);
