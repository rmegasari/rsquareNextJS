 ...
 * @param {boolean} includeInactive - Include inactive products (for admin)
 */
export async function getAllProductsFromSupabase(includeInactive = false) {
  try {
    let query = supabase
      .from("products")
      .select("*");

    // Filter only active products for public pages
    if (!includeInactive) {
      // Include products where active is true OR null (backward compatibility)
      query = query.or("active.eq.true,active.is.null");
    }

    // Try to order by order_number, fallback to created_at if column doesn't exist
    let { data: products, error } = await query.order("order_number", { ascending: true });

    // If order_number column doesn't exist, try ordering by created_at
    if (error && error.code === '42703') {
      console.warn("⚠️ order_number column not found, falling back to created_at");
      const fallbackQuery = supabase.from("products").select("*");
      if (!includeInactive) {
        fallbackQuery.or("active.eq.true,active.is.null");
      }
 ...
 */
export async function updateActiveStatusSupabase(productId, active) {
  try {
    const { error } = await supabase
      .from("products")
    ...
    console.error("Error updating active status in Supabase:", error);
    throw error;
  }
}
